...
from fast_pytorch_kmeans import KMeans as KMeans_GPU
...
def harmonize(
    ...
    if verbose:
        print("\tInitialising is commencing.")
...
def initialize_centroids(
    ...
    with threadpool_limits(limits = n_jobs):
        if device_type == "cpu":
            ...
            kmeans = KMeans_GPU(n_clusters = n_clusters, verbose = 1, max_iter = 30)
            kmeans.fit(Z_norm)
            Y = torch.tensor(kmeans.centroids, dtype=torch.float, device=device_type)
