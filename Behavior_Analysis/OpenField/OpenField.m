function OF = OpenField(video_fn, video_folderpath, loop_counter, loop_total)
%function OpenField()
...
%Hi user! This script is linked to OpenField_Automator.m.
%Please run that code if you wish to loop through all of your videos at
%once, otherwise for a more manual experience please use MouseActivity5.m.



%note to Becky: please fix thigmotaxis ratio(innerT/outerT % if greater than 0) + colours.
...
%clear; % erase all existing variables
    %disp(video_fn);
    %disp(video_folderpath);
    %disp(class(video_fn));
    %disp(class(video_folderpath));
    %[filename, dirpath]% = {video_fn, video_folderpath};%uigetfile('*.mov;*.wmv;*.mp4;*.avi','Open video');
    filename = char(video_fn);
    dirpath = char(video_folderpath);
    %disp([filename ' : ' dirpath]);
...
%         dlg_title = 'Collect movie processing parameters';
        %num_lines = 1;
        %defaultans = {th,'20','2','1',num2str(nframes),'1'};
        %defaultans = {th,'20','1','1',num2str(nframes),'1'};
        %answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
        %threshold = str2double(answer{1}); % default 0.70; % the threshold is a key parameter for successful segmentation
        threshold = 0.80; %Auto 0.80, can be tweaked depending on the performance experienced
        objpixels = 25;%floor(str2double(answer{2})); % the minimal pixel size of the mouse, default is 50
        MouseN = 1;%floor(str2double(answer{3})); % the number of mouse in the movie, default is 2
        StartFrame = 1;%floor(str2double(answer{4})); % the first frame to be analyzed, default is 1
        LastFrame = 1800;%floor(str2double(answer{5})); % the last frame to be analyzed, default is the maximum frame number
        if LastFrame > nframes
            LastFrame = nframes;
        end
        Step = 1;%floor(str2double(answer{6})); % the frame steps to be analyzed (e.g. analyze every 'x' frame), default is 1
        logf(['\nDEFAULT VALUES' '\nThreshold: ' int2str(threshold) '\nobjpixels: ' int2str(objpixels) '\nNumber of Mice: ' int2str(MouseN) '\nStart Frame: ' int2str(StartFrame) '\nLast Frame: ' int2str(LastFrame) '\nStep Count: ' int2str(Step)]);
        close all;
        ...
        h0 = 710;
        for n = 1:MouseN
            logf(['Please draw the arena for mouse ' loop_counter '.']);
            arena = drawrectangle('Position',[184 7 w0 h0], 'LineWidth',1); 
            %text_x1 = floor(((video_obj.Width/5)/video_obj.Width));
            %text_y1 = floor((video_obj.Height/10)/video_obj.Height);
            %dim = [text_x1 (text_x1+(text_x1/5)) text_y1 (text_y1+(text_y1/5))];
            %str1 = {'threshold: '+threshold,'minPixels: '+objpixels, 'numMice: '+MouseN, 'startFrame: '+StartFrame,'endFrame: '+ LastFrame, 'stepSize: '+Step};
            %mouse_legend = insertText('textbox',dim,'String',str1, 'FitBoxToText','on'); %display the info of the default settings along with the x/y coords of the rect
        ...
        f = waitbar(0,'1','Name',['Tracking mouse ' loop_counter ' of ' loop_total]);
        %disp(position);                
        close all;
    ...
    if exist(resultpath, 'file') == 2 && exist([dirpath video_name '/' [video_name '_result.xls']], 'file') ~= 2
        ...
        logf('Tracking and analysis have been completed.');
         ...
         logf('Rendering mouse tracking video...');
        ...
        if exist(trackmoviepath, 'file') ~= 2
            ...
            numFrames = 100;%change this to render a longer video, takes considerably longer for more frames
            F(min(length(x),numFrames)) = struct('cdata',[],'colormap',[]);

            colors = ['b', 'r', 'g', 'm', 'y'];
            f = waitbar(0,'1','Name','Writing Video');
            for i=1:min(length(x),numFrames) %here you can change to for i=1:length(x)
                %waitbar(i/numFrames,f,sprintf(['Progress: ' num2str((i/numFrames)*100) '%%']))
                waitbar(i/numFrames,f,sprintf(['Progress: ' num2str(i) '/' num2str(numFrames)]));
                %disp(i)
...
%                     text(40+270*(mn-1),400,['mouse area: ' num2str(m1_area{mn}(i,1)) ' ecc: ' num2str(m1_ecc{mn}(i,1))],'Color','r','FontSize',12,'FontWeight','bold')
            ...
            delete(f);
            logf('Video Rendered, saving.... (This process can take a while depending on the size of the video)');
...
%         end
        ...
        if (loop_counter == loop_total)
            prompt = sprintf(['Folder Analysis Completed, Please Rerun the program to analyse another folder.']);
            button = questdlg(prompt,'Finished','Close','Close'); 
            if strcmp(button,'Close')
                return;
            end
        else 
            prompt = sprintf(['WARNING: Choosing to close MATLAB will close the entire application..''Would you like to continue?...']);
            button = questdlg(prompt,['Currently Finished ' loop_counter ' of ' loop_total ' Mice.'],'Continue', 'Close MATLAB','Continue'); 
...
function analysis(filename1, dirpath1)
    ...
    %disp(filepath);
    [u1 video_name u2] = fileparts(filepath);
    %disp(['testing testing testig' u1 '    ' video_name '    ' u2]);            
    result_name = [video_name '_result.mat'];
    
    resultpath = [u1 '\' video_name '\' result_name];
    %disp(resultpath);
...
% hold off
    ...
    set(gcf, 'Position', [0, 0.04, 450, 900], 'Visible', 'off');
    saveas(gcf,[imgfilename1 '.tif'],'tiff');
    ...
    for i=1:mouseN
        ...
        arenaW = arena{i}(:,3);
        arenaH = arena{i}(:,4);
        pixels = 385/arenaW; %pixel size in mm; the arena is 12 x 12 inches. CONVERTED TO 380mm (diameter of arena)
        peripheral = floor(arenaW./380*2); % 2 inches
        ...
        % disp("centerx: "+(arena{i}(:,1)+floor(arenaW/2))+" centery: "+(arena{i}(:,2)+floor(arenaH/2))+" radius: "+floor(arenaW/2)*0.75);
        poly_inner = nsidedpoly(20,'Center',[(arena{i}(:,1)+floor(arenaW/2)) (arena{i}(:,2)+floor(arenaH/2))],'Radius',floor(arenaW/2)*0.66);
        poly_outer = nsidedpoly(20,'Center',[(arena{i}(:,1)+floor(arenaW/2)) (arena{i}(:,2)+floor(arenaH/2))],'Radius',floor(arenaW/2));
        
        [poly_x,poly_y] = boundary(poly_inner);
        xv = poly_x;
        yv = poly_y;
        % disp("Px: "+xv);
        % disp("Py: "+yv);
        ...
        distance1 = [0];
        speed = [0];
        %speed = distance1/timesin1;
        acceleration = [0];
        %acceleration = speed/timesin1;
        ...
        for k = 2:length(pos{i})
              ...
              speed = [speed; S1];
              A1 = S1/ctimearray(k);
              acceleration = [acceleration; A1];
        end
        
        %horzcat = concatenate arrays horizontally 
        titlerow = horzcat({'Time (s)'},{[mouse '_X']},{[mouse '_Y']},{[mouse '_PathL (mm)']},{[mouse '_Distance(m)']},{[mouse '_Speed (m/s)']}, {[mouse '_Acceleration (m/s^2)']});
        finaldata = horzcat(ctimearray,xq1,yq1,pathl1,distance1,speed,acceleration);
        
        %added in
        TotDist = distance1(end);
        MeanDist = mean(distance1);
        ...
        CountRestTime = sum(pathl1(:)==0); %time at rest is when path ==0
        RestTime = CountRestTime./30; %time spent at rest (CountRestTime as a function of time)
        TimeInner1 = timesin1.*step./ frameRate;
        TimeOuter1 = totalframe.*step./frameRate - TimeInner1;
        Thigmotaxis1 = 1-timesin1./length(x);
        summarytitle = [{'FileName'},{[mouse '_TotalDistance (m)']},{[mouse '_MeanDistance (m)']},{[mouse '_MeanVelocity (m/s)']},{[mouse '_MaxVelocity (m/s)']},{[mouse '_MinVelocity (m/s)']},{[mouse '_MeanAcceleration (m/s^2)']},{[mouse '_MaxAcceleration (m/s^2)']},{[mouse '_MinAcceleration (m/s^2)']},{[mouse '_RestingTime']},{[mouse '_Outer (s)']},{[mouse '_Inner (s)']},{[mouse '_Thigmotaxis']}];
        summaryresults = [{[dirpath1 '\' video_name '.mov']},TotDist,MeanDist,MeanSpeed,MaxSpeed,MinSpeed,MeanAcc,MaxAcc,MinAcc,RestTime,TimeOuter1,TimeInner1,Thigmotaxis1];

        %disp([dirpath1 '\' video_name '/' xlsfilename]);
...
%     plot(finaldata1{1}(:,1),traveldistanceplot,'LineWidth',2);
    ...
    set(gcf,'Units','Normalized','Position',[0 0 1 0.5],'PaperPositionMode','auto','PaperSize',[14 14], 'Visible', 'off');
    % title(sprintf(video_name));
    title([video_name 'Cumulative Distance'],'Interpreter','none');

    imgfilename = [dirpath1 '\' video_name '/' video_name '_summary'];
    saveas(gcf,[imgfilename '.tif'],'tiff');
    hold off
    %delete(hsum);
